...
def log_image_with_masks_without_box(
    tag: str,
    image: torch.Tensor,
    bboxes: torch.Tensor,
    masks: torch.Tensor,
    ...
    if bboxes is None:
        n = labels.shape[0]
        bboxes = labels.new_zeros((n, 4)).long()
    #print("+"*100)
    #print("backend", backend) # file
...
def isVisualbyCount(interval: int = 1000):
    rank, _ = get_dist_info()
    if rank != 0:
        return 0
    _, key = _find_caller()
...
def isVisualbyCount_return_count(interval: int = 1000):
    rank, _ = get_dist_info()
    if rank != 0:
        return 0, 0
    _, key = _find_caller()
...
#-------#
    ...
    #print("_log_counter[key]", _log_counter[key])
    
    if not (interval == 1 or _log_counter[key] % interval == 1):
        return 0, _log_counter[key]
    return 1, _log_counter[key]