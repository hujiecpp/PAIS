...
class DoubleMeanTeacher(Hook):
    ...
    def before_run(self, runner):
        ...
        if is_module_wrapper(model):
            model = model.module
        assert hasattr(model, "teacher1")
        assert hasattr(model, "student1")
        assert hasattr(model, "student2")
        assert hasattr(model, "teacher2")
        # only do it at initial stage
        if runner.iter == 0:
            log_every_n("Clone all parameters of studenti to teacheri...")
    ...
    def momentum_update(self, model, momentum):
        for (src_name, src_parm), (tgt_name, tgt_parm) in zip(
            model.student1.named_parameters(), model.teacher1.named_parameters()
        ):
