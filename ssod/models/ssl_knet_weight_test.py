...
from .utils import Transform2D, filter_invalid_3, cut_patch
...
class SslKnet_weight_test(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SslKnet_weight_test, self).__init__(
...
#!增加cutout
        #batch_patch = []
        #num_img = len(gt_masks_tensor)
        #for i in range(num_img): 
        _, h, w = gt_masks_tensor[0].shape
        patch = cut_patch(h, w, 
                          n_iterations=(1, 5),
                          size=[0, 0.2],
                          squared=True)        
        ...
        rpn_results = self.student.rpn_head.forward_train(x, img_metas, gt_masks,
                                                  gt_labels, gt_scores, gt_ious, patches=patch)
