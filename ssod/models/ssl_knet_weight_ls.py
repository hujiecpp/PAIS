...
from ssod.utils import log_image_with_boxes, log_every_n, log_image_with_masks, log_image_with_masks_without_box, isVisualbyCount, log_image_with_boxes
...
class SslKnet_weight_ls(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SslKnet_weight_ls, self).__init__(
...
#----------------------------------#
        ...
        rpn_results = self.student.rpn_head.forward_train_unsup(x, img_metas, gt_masks, gt_box_mask, 
                                                                gt_labels, gt_scores, gt_ious, levelset_img=levelset_img)
        (rpn_losses, proposal_feats, x_feats, mask_preds, cls_scores, levelset_feat) = rpn_results
        
        loss_rpn_seg = rpn_losses['loss_rpn_seg']
        rpn_losses['loss_rpn_seg'] = loss_rpn_seg * 0
        '''
        loss_rpn_mask = rpn_losses['loss_rpn_mask']
        rpn_losses['loss_rpn_mask'] = loss_rpn_mask * 0
        '''
        loss_rpn_dice = rpn_losses['loss_rpn_dice']
        rpn_losses['loss_rpn_dice'] = loss_rpn_dice * 0
...
#------------------------------------#
        ...
        losses["s0_loss_dice"] = dice_loss * 0
        dice_loss = losses["s1_loss_dice"]
        losses["s1_loss_dice"] = dice_loss * 0
        dice_loss = losses["s2_loss_dice"]
        losses["s2_loss_dice"] = dice_loss * 0
