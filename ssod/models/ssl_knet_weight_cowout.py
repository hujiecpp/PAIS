...
class SslKnet_weight_cowout(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SslKnet_weight_cowout, self).__init__(
...
#对伪mask也进行cowout        
        ...
        for i in range(num_img):
            ...
            #print("aug_info", img_metas[i]["aug_info"])
            
            
            for dic in img_metas[i]["aug_info"]:
                if dic['type'] == 'CutMix':
                    patches = dic["patches"]
                    mixes = dic["mixes"]
                    break
            ...
            for patch, mix in zip(patches, mixes):
                ...
                x1, y1, x2, y2 = patch
                for j in range(c):
                    pesudo_image[j, y1:y2, x1:x2] = mix  
                tmp[pesudo_image==1] = 0
                masks = BitmapMasks(tmp, masks.height, masks.width)
                #print("masks.masks", masks.masks)
            pseudo_masks[i] = masks
