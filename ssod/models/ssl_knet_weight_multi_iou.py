...
from .utils import Transform2D, filter_invalid_3, filter_invalid_4
...
class SslKnet_weight_multi_iou(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SslKnet_weight_multi_iou, self).__init__(
...
#！由于两个通道的数据增强不一致，需要将teacher的mask变换到student上，mask的变换----------------------#
        ...
        gt_mask_thr = teacher_info["det_mask_thr"]
...
#-----------------------------------------------------#
                mask_thr = gt_mask_thr[i].cpu().detach()
                #print("mask_thr", mask_thr, mask_thr.shape)
                n, h, w = mask_vis.shape
                mask_thr = mask_thr.unsqueeze(-1).expand(n, h, w)
                #print("mask_thr", mask_thr, mask_thr.shape)
                mask_vis = mask_vis > mask_thr
...
#二值化     
            ...
            mask_thr = mask_thr.unsqueeze(-1).expand(n, h, w).to(mask_tensor.device)
            #print("mask_thr", mask_thr, mask_thr.shape)
            mask_tensor = mask_tensor > mask_thr
...
#--------------------------------------------#
        ...
        seg_results, label_results, score_results, iou_results, mask_thr = self.teacher.roi_head.teacher_test(x_feats, proposal_feats, mask_preds, cls_scores, img_metas)
        #end2 = time.time()
...
#-两种过滤方式。带iou的和不带iou的
#----------------------------------#
        det_masks, det_labels, det_scores, det_ious, det_mask_thr = list(   
            zip(
                *[
                    filter_invalid_4(
                        mask=seg_result,
                        ...
                        score=score_result,
                        iou=iou_result,
                        mask_thr=mask_thr_result,
                        thr=thr,
                        iou_thr=iou_thr
                    )
                    for seg_result, label_result, score_result, iou_result, mask_thr_result in zip(
                        seg_results, label_results, score_results, iou_results, mask_thr
                    )
                ]
            )
        )
...
#---------------------------------------------#        
        ...
        teacher_info["det_mask_thr"] = det_mask_thr
