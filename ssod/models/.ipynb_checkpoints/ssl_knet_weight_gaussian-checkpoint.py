...
class SslKnet_weight_gaussian(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SslKnet_weight_gaussian, self).__init__(
...
#可视化---------------------------------------    不匹配 
            ...
            if flag == 1:            
                ...
                mask_vis_1 = mask_vis > 0.1
                mask_vis_2 = mask_vis > 0.2
                mask_vis_3 = mask_vis > 0.3
                mask_vis_4 = mask_vis > 0.4
                mask_vis_5 = mask_vis > 0.5
...
#尝试高斯模糊----------------------------------#
        ...
        #print("gt_masks_tensor[0].shape:", gt_masks_tensor[0].shape, gt_masks_tensor[0].dtype, gt_masks_tensor[0])  #2 torch.Size([14, 200, 336]) float
        

        num_img = len(img_metas)
        for i in range(num_img):
            
            mask = gt_masks_tensor[i].cpu().detach()
            mask = mask.numpy()
            
            
            if mask.shape[0] != 0:
                mask = cv2.GaussianBlur(src=mask,ksize=(3,3),sigmaX=1.0, sigmaY=1.0)
            gt_mask = torch.from_numpy(mask).float().to(gt_masks_tensor[0].device)
            #mask.to_tensor(torch.float, gt_masks_tensor[0].device)
            #np.set_printoptions(threshold=np.inf)
            #print("gt_mask", gt_mask)
            #gt_mask = gt_mask > 0.2
            
            gt_masks_tensor[i] = gt_mask
          
            gt_label = gt_labels[i].cpu().detach().numpy()
            gt_mask =  gt_masks_tensor[i]
            #gt_mask = gt_mask > 0.1
            gt_score = gt_scores[i].cpu().detach()
            gt_iou = gt_ious[i].cpu().detach()
            print("iter"+"="*100)
            print("gt_score", gt_score)
            print("gt_iou", gt_iou)
            coco_class = np.array(self.CLASSES)
            print("gt_label", coco_class[gt_label])
            
#可视化---------------------------------------    不匹配 
            #如果是interval的倍数，才可视化图片
            if flag == 1:            
                image_vis = img[i].cpu().detach().float()
                #print("image_vis.shape", image_vis.shape)
                '''
                image_vis = F.interpolate(
                                image_vis[None], (assign_H, assign_W),
                                mode='bilinear',
                                align_corners=False)[0]
                '''
                
                mask_vis = gt_mask.cpu().detach().float()
                '''
                print("det"+"-"*100)
                import numpy as np
                np.set_printoptions(threshold=np.inf)
                ...
                print("mask", mm.shape, mm)
                '''
                if mask_vis.shape[0] == 0:
                    mask_vis = mask_vis.new_zeros(
                            (mask_vis.size(0), pad_H, pad_W))
                else:
                    mask_vis = F.interpolate(
                                mask_vis[None], (pad_H, pad_W),
                                mode='bilinear',
                                align_corners=False)[0]
