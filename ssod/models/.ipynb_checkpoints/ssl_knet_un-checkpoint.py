...
class SslKnet_un(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SslKnet_un, self).__init__(
...
#---------------------------------------# 
    ...
    def foward_unsup_train(self, teacher_data, student_data):
        ...
        return self.compute_pseudo_label_loss(student_data["img"], student_data["img_metas"],  teacher_info, student_info)

#!修改
#-----------
    def compute_pseudo_label_loss(self, img, img_metas, teacher_info, student_info):
...
#----------------------------------#
        ...
        stu_masks = student_info["det_masks"]
        stu_labels = student_info["det_labels"]
        
        print("+"*100)
        print("stu_masks.device", stu_masks.device)
        print("gt_masks.device", gt_masks.device)
        
        for i in range(num_imgs):
            assign_result = self.student.assigner.assign(stu_masks[i],
                                                         stu_labels[i],
...
#-------------------------------------------------#
    ...
    def extract_student_info(self, img, img_metas, **kwargs):
        ...
        if isinstance(self.train_cfg.stu_inference_score_thr, float): 
            thr = self.train_cfg.stu_inference_score_thr
        else:
