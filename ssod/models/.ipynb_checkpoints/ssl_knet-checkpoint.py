...
from ssod.utils import log_image_with_boxes, log_every_n, log_image_with_masks, log_image_with_masks_without_box
...
class SslKnet(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SslKnet, self).__init__(
...
#---------------------------------------#            
    ...
    def foward_unsup_train(self, teacher_data, student_data):
        ...
        #print("+"*100) 
        #print("student_data_img.shape", student_data["img"].shape) #有2，有1
...
#------------------------------------------#
        ...
        print("变换矩阵", M)
        print("="*200)
        print("pseudo_masks", pseudo_masks[0].masks)
...
#查看pad之前的mask大小-------------------该mask已经经过数据增强的变化，且变化到img_shape的size，但是跟img.shape是不一样的   
            '''
            print("stu中的img"+"="*50)
            print("img_meta['img_shape']", img_metas[0]["img_shape"])
            print("img_metas['ori_shape']", img_metas[0]['ori_shape'])
            print("img_metas['pad_shape']", img_metas[0]['pad_shape'])            
            
            print("img.shape", img.shape)
            print("img_meta['filename']", img_metas[0]["filename"])

            print("pad之前的"+"|"*100)
            print("img.shape", img.shape)
            print("mask_tensor", mask_tensor.shape)
            print("img_meta['img_shape']", img_metas[0]["img_shape"])
            print("img_meta['batch_input_shape']", img_metas[0]["batch_input_shape"])
            '''
...
#----------------------------------#
        ...
        print("gt_masks", gt_masks[0])
        '''
        #from mmdet.apis import inference_detector, show_result_pyplot
        #show_result_pyplot(model, img[0], result[0])
...
#!!!!!!!!!
    def extract_teacher_info(self, img, img_metas, proposals=None, **kwargs):
        ...
        det_masks, det_labels, det_scores = list(   
            zip(
                *[
                    filter_invalid_1(
                        mask=seg_result,
...
#可视化teacher生成的mask在teacher上    
        '''
        print("tea生成的mask的尺度和img的尺度"+"="*100)
        print("det_masks[0].masks.shape", det_masks[0].masks.shape)
        print("img.shape", img.shape)
        
        print("tea的img"+"+"*50)
        print("img_metas['img_shape']", img_metas[0]['img_shape'])
        print("img_metas['ori_shape']", img_metas[0]['ori_shape'])
        print("img_metas['pad_shape']", img_metas[0]['pad_shape'])
        print("img_metas['batch_input_shape']", img_metas[0]['batch_input_shape'])
        print("img.shape", img.shape)
        print("img_metas['filename']", img_metas[0]["filename"])
        '''
        pad_H, pad_W = img_metas[0]['batch_input_shape']
        
        mask_tensor = det_masks[0].to_tensor(torch.float, det_labels[0].device)
        if det_masks[0].width != pad_W or det_masks[0].height != pad_H:
            pad_wh = (0, pad_W - det_masks[0].width, 0, pad_H - det_masks[0].height)
            mask_tensor = F.pad(mask_tensor, pad_wh, value=0) 
            mask_tensor = mask_tensor > 0.5  #二值化
            #print("是否二值化了"+"="*100)   #没有二值化
            #print("mask_tensor", mask_tensor)
#测试mask的变换是否有问题        --------------尺度对上了
        '''
        print("tea_pad之后的mask"+"/"*100)
...
#可视化---------------------------------不对齐
        ...
        teacher_info["score_results"] = det_scores
