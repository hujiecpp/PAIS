...
from .utils import Transform2D, filter_invalid_3, Transform2D_1, Transform2D_2
...
class SslKnet_weight_sc(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SslKnet_weight_sc, self).__init__(
...
#---------------------------------------# 
    ...
    def foward_unsup_train(self, teacher_data, student_data):
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                teacher_info_scale = self.extract_teacher_info_scale(
                teacher_data["img"][
                    torch.Tensor(tidx).to(teacher_data["img"].device).long()
                ],
                [teacher_data["img_metas"][idx] for idx in tidx],
        ...
        return self.compute_pseudo_label_loss(student_data["img"], student_data["img_metas"],  teacher_info, teacher_info_scale)

#!修改
#-----------
    def compute_pseudo_label_loss(self, img, img_metas, teacher_info, teacher_info_scale):
...
#！由于两个通道的数据增强不一致，需要将teacher的mask变换到student上，mask的变换----------------------#
        pseudo_masks = self._transform_mask_1(
        ...
        pseudo_masks_scale = self._transform_mask_2(
            teacher_info_scale["det_masks"],   #list(bitmapmasks)    3, 
            M,
            [meta["img_shape"] for meta in img_metas],
        )
        gt_labels_scale = teacher_info_scale["det_labels"]
...
#可视化下采样图片进行预测后的mask-------------#
        for i, gt_mask in enumerate(pseudo_masks_scale):
            mask_tensor = gt_mask.to_tensor(torch.float, gt_labels_scale[0].device)
            ...
            if flag == 1:            
                ...
                mask_vis = mask_vis > 0.5
                label_vis = gt_labels_scale[i].cpu().detach()
        ...
        gt_masks_scale = gt_masks_tensor_scale
        gt_scores_scale =  teacher_info_scale["det_scores"]
        gt_ious_scale = teacher_info_scale["det_ious"] 
        
        rpn_results = self.student.rpn_head.forward_train(x, img_metas, gt_masks,
                                                  gt_labels, gt_scores, gt_ious)
        '''
        rpn_results_scale = self.student.rpn_head.forward_train(x, img_metas, gt_masks_scale,
                                                  gt_labels_scale, gt_scores_scale, gt_ious_scale, is_scale='_scale')
        '''
        (rpn_losses, proposal_feats, x_feats, mask_preds,
         cls_scores) = rpn_results
        #(rpn_losses_scale, proposal_feats_scale, x_feats_scale, mask_preds_scale,
        #cls_scores_scale) = rpn_results_scale
        
        loss_rpn_seg = rpn_losses['loss_rpn_seg']
        rpn_losses['loss_rpn_seg'] = loss_rpn_seg * 0

        #loss_rpn_seg_scale = rpn_losses_scale['loss_rpn_seg_scale']
        #rpn_losses_scale['loss_rpn_seg_scale'] = loss_rpn_seg_scale * 0
#------------------------------------#
        ...
        '''
        losses_scale = self.student.roi_head.forward_train(
            x_feats_scale,
            proposal_feats_scale,
            mask_preds_scale,
            cls_scores_scale,
            img_metas,
            gt_masks_scale,
            ...
            is_scale='_scale',
            imgs_whwh=None)
        iou_loss_scale = losses_scale["s0_loss_iou_scale"]
        losses_scale["s0_loss_iou_scale"] = iou_loss_scale * 0
        iou_loss_scale = losses_scale["s1_loss_iou_scale"]
        losses_scale["s1_loss_iou_scale"] = iou_loss_scale * 0        
        iou_loss_scale = losses_scale["s2_loss_iou_scale"]
        losses_scale["s2_loss_iou_scale"] = iou_loss_scale * 0
        
        '''
        losses.update(rpn_losses)
        #losses.update(losses_scale)
        #losses.update(rpn_losses_scale)
        return losses        
...
#！加一个函数，类似于_transform_bbox，用于mask的变换
    ...
    def _transform_mask_1(self, masks, trans_mat, max_shape):
        masks = Transform2D_1.transform_masks(masks, trans_mat, max_shape)
        return masks
    
    def _transform_mask_2(self, masks, trans_mat, max_shape):
        masks = Transform2D_2.transform_masks(masks, trans_mat, max_shape)
        return masks
...
#---------------------------------------------#        
    ...
    def extract_teacher_info_scale(self, img, img_metas, proposals=None, **kwargs):
        teacher_info = {}
        _, _, h, w = img.shape
        img_downsample = F.interpolate(img, (h//2, w//2), mode='bilinear', align_corners=False)
        pad_wh = (0, w//2, 0, h//2)
        img_downsample = F.pad(img_downsample, pad_wh, value=0)

        feat = self.teacher.extract_feat(img_downsample)
        teacher_info["backbone_feature"] = feat
        #不需要保存teacher的proposal
...
#--------------------------------------------#
        #seg_results, label_results, score_results, _ = self.teacher.roi_head.teacher_test(x_feats, proposal_feats, mask_preds, cls_scores, img_metas)
        seg_results, label_results, score_results, iou_results = self.teacher.roi_head.teacher_test_scale(x_feats, proposal_feats, mask_preds, cls_scores, img_metas)
        #end2 = time.time()
        #print("test_cost:", end2 - start2)
