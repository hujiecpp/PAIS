...
#----------------------------#
        ...
        num_pos = pos_inds.sum().float()        #将true的数量统计出来
...
#根据loss大小去掉一些mask
                '''
                print("head" + "-"*100)
                #print("pos_inds", pos_inds, pos_inds.shape) #true, false 100
                print("cls_score.detach().view(num_preds, -1)", cls_score.detach().view(num_preds, -1))
                print("labels", labels, labels.shape)
                print("pos_inds.any()", pos_inds.any())
                print("flag", flag)
                '''
                if flag == 0:
                    #if pos_inds.any():
                    loss_cls = []
                    #bool_pos_inds = pos_inds.type(torch.bool).detach()

                    #print("bool_pos_inds", bool_pos_inds, bool_pos_inds.shape) #跟pos_inds一样

                    pos_cls_score = cls_score.detach().view(num_preds, -1)[pos_inds]
                    pos_label = labels.detach()[pos_inds]
                    pos_label_weights = label_weights.detach()[pos_inds]
                    num_pos_cls = pos_cls_score.shape[0]
                    num_pos_label = pos_label.shape[0]
                    assert num_pos_cls == num_pos_label  
                    for i in range(num_pos_cls):
                        ind = torch.from_numpy(np.array([i])).to(cls_score.device)
                        loss_cls.append(
                            self.loss_cls(
                                pos_cls_score[ind],
                                pos_label[ind],
                                pos_label_weights[ind],
                                avg_factor=None,
                                reduction_override=reduction_override))
                    num_save = round(num_pos_cls*0.9)

                    #print("num_save", num_save)
                    #print("loss_cls", loss_cls)

                    index = np.array(heapq.nsmallest(num_save, range(num_pos_cls), loss_cls.__getitem__))
                    index = torch.from_numpy(index).to(cls_score.device)

                    num_save_tensor = torch.tensor(num_save).float().to(cls_score.device)

                    avg_factor_1 = reduce_mean(num_save_tensor).clamp_(min=1.0)
                    #print("avg_factor", avg_factor, avg_factor.requires_grad) #False
                    #print("index", index, index.requires_grad) #False
                    #print("avg_factor_1", avg_factor_1)
                    
                    neg_cls_score = cls_score.view(num_preds, -1)[pos_inds.logical_not()]
                    neg_label = labels[pos_inds.logical_not()]
                    neg_label_weights = label_weights[pos_inds.logical_not()]
                    #print("pos_inds.logical_not()", pos_inds.logical_not())
                    
                    if num_save==0:
                        cls_score_1 = neg_cls_score
                        labels_1 = neg_label
                        label_weights_1 = neg_label_weights
                    else:
                        pos_cls_score_1 = pos_cls_score[index]
                        pos_label_1 = pos_label[index]
                        pos_label_weights_1 = pos_label_weights[index]
                        
                        cls_score_1 = torch.cat((pos_cls_score_1, neg_cls_score))
                        labels_1 = torch.cat((pos_label_1, neg_label))
                        label_weights_1 = torch.cat((pos_label_weights_1, neg_label_weights))
                    #print("cls_score", cls_score.requires_grad) #True
                    #print("cls_score_1", cls_score_1, cls_score_1.shape)
                    ...
                    losses['loss_cls'] = self.loss_cls(
                        ...
                        labels_1,
                        label_weights_1,
                        avg_factor=avg_factor_1,
                        reduction_override=reduction_override)     

                    losses['pos_acc'] = accuracy(
                        pos_cls_score, pos_label)
                    '''
                    else:
                        losses['loss_cls'] = self.loss_cls(
                    ...
                    print("before"+"|"*50)
                    print("avg_factor", avg_factor, avg_factor.requires_grad)  #False
                    print("pos_inds", pos_inds) #false, true
                    '''
                    """
                    loss_cls = []
                    ...
                    with torch.no_grad():
                        ...
                        #print("head"+"-"*50)
                        '''
                        print("avg_factor", avg_factor) #tensor(5.2500, device='cuda:3')
                        print("reduction_override", reduction_override) #None
                        print("pos_inds", pos_inds.shape, pos_inds) #torch.Size([100]),二值
                        print("labels", labels.shape, labels)
                        print("cls_score", cls_score.shape)
                        print("label_weights", label_weights)#都是1
                        '''
                        #print("num_pos", num_pos) #tensor(3., device='cuda:0')
                        
                        #print("bool_pos_inds", bool_pos_inds)
                        
                        pos_cls_score = cls_score.view(num_preds, -1)[bool_pos_inds]
                        pos_label = labels[bool_pos_inds]
                        pos_label_weights = label_weights[bool_pos_inds]
                        ...
                        assert num_pos_cls == num_pos_label   
                        #assert num_pos_cls == int(num_pos)
                        ...
                        for i in range(num_pos_cls):
                            ...
                            loss_cls.append(
                                self.loss_cls(
                                    ...
                                    avg_factor=None,
                                    reduction_override=reduction_override))
                        #print("loss_cls", loss_cls) #[tensor(0.0067, device='cuda:1')]
                        num_save = round(num_pos_cls*0.9)
                        index = np.array(heapq.nsmallest(num_save, range(num_pos_cls), loss_cls.__getitem__))
                        index = torch.from_numpy(index).to(cls_score.device)
                    ...
                    avg_factor = reduce_mean(num_save_tensor).clamp_(min=1.0)
                    #print("avg_factor", avg_factor, avg_factor.requires_grad) #False
                    #print("index", index, index.requires_grad) #False
                    
                    pos_cls_score = cls_score.view(num_preds, -1)[pos_inds][index]
                    pos_label = labels[pos_inds][index]
                    pos_label_weights = label_weights[pos_inds][index]
                    neg_cls_score = cls_score.view(num_preds, -1)[bool_pos_inds.logical_not()]
                    neg_label = labels[bool_pos_inds.logical_not()]
                    neg_label_weights = label_weights[bool_pos_inds.logical_not()]
                    
                    cls_score = torch.cat((pos_cls_score, neg_cls_score))
                    labels = torch.cat((pos_label, neg_label))
                    label_weights = torch.cat((pos_label_weights, neg_label_weights))
                    #print("cls_score", cls_score.requires_grad) #True
...
#测试，尺度是否一致
                ...
                if flag == 0:
                    print("roi_head"+"+"*100)
                    print("pos_mask_pred.shape", pos_mask_pred.shape)
                    print("pos_mask_targets.shape", pos_mask_targets.shape)                
                '''
#根据loss大小随机去掉一些loss，仅用于无标签图片
                #print("head")
                #print("bool_pos_inds", bool_pos_inds.requires_grad) #False
                #print("flag", flag)
                if flag == 0:
                    pos_mask_pred_1 = pos_mask_pred.detach()
                    pos_mask_targets_1 = pos_mask_targets.detach()
                    num_pos_mask_pred = pos_mask_pred_1.shape[0]
                    num_pos_mask_targets = pos_mask_targets_1.shape[0]

                    assert num_pos_mask_pred == num_pos_mask_targets
                    loss_rpn_mask = []
                    loss_rpn_dice = []

                    for i in range(num_pos_mask_pred):
                        loss_rpn_mask.append(loss_mask(pos_mask_pred_1[i], pos_mask_targets_1[i]))
                        loss_rpn_dice.append(self.loss_dice(pos_mask_pred_1[i], pos_mask_targets_1[i]))
                    #loss_rpn_mask = np.array(loss_rpn_mask)
                    #print("loss_rpn_mask", loss_rpn_mask[0], loss_rpn_mask[0].requires_grad)
                    
                    num_save = round(num_pos_mask_pred*0.9)
                    index = np.array(heapq.nsmallest(num_save, range(num_pos_mask_pred), loss_rpn_mask.__getitem__))

                    index_1 = np.array(heapq.nsmallest(num_save, range(num_pos_mask_pred), loss_rpn_dice.__getitem__))

                    index = torch.from_numpy(index).to(pos_mask_pred.device)
                    index_1 = torch.from_numpy(index_1).to(pos_mask_pred.device)

                    final_pos_mask_pred =  pos_mask_pred[index]
                    final_pos_mask_targets = pos_mask_targets[index]

                    #print("final_pos_mask_pred", final_pos_mask_pred.requires_grad)

                    losses['loss_mask'] = loss_mask(final_pos_mask_pred,
                                                             final_pos_mask_targets)

                    final_pos_mask_pred_1 =  pos_mask_pred[index_1]
                    final_pos_mask_targets_1 = pos_mask_targets[index_1]            

                    losses['loss_dice'] = self.loss_dice(final_pos_mask_pred_1,
                                                             final_pos_mask_targets_1)                   
                
                else:
                    losses['loss_mask'] = loss_mask(pos_mask_pred,
                                                         pos_mask_targets)
