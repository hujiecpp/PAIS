...
@PIPELINES.register_module()
class CutMix(GeometricAugmentation):
    def __init__(
        self,
        n_iterations=None,
        size=None,
    ...
    def get_magnitude(self, results):
        ...
        if self.random_magnitude:
            ...
            for i in range(n_iterations):
                ...
                img_size = (ph, pw)
                mix = self.generate_mixing_mask_1(img_size, sigma, 0.5)
                mixes.append(mix)
            magnitude["patches"] = patches
            magnitude["mixes"] = mixes
            
        else:
            assert self.patches is not None
            magnitude["patches"] = self.patches
            magnitude["mixes"] = self.mixes
    ...
    def apply(self, results, patches: list, mixes):

#-------------------------#
        #print(patches)        
        
        for patch, mix in zip(patches, mixes):
            h, w, c = results["img_shape"]
            self._erase_image(results, mix, patch, fill_val=self.img_fill_val)
            self._erase_mask(results, mix, patch)
            self._erase_seg(results, mix, patch, fill_val=self.seg_ignore_label)
        return results

    
    def generate_mixing_mask_1(self, img_size, sigma, p):
    ...
    def _erase_image(self, results, mix, patch, fill_val=128):
        for key in results.get("img_fields", ["img"]):
            tmp = results[key].copy()
            #print(tmp.shape)
            h, w, c = tmp.shape
            pesudo_image = np.zeros((h, w, c))
            x1, y1, x2, y2 = patch
            for i in range(c):
                pesudo_image[y1:y2, x1:x2, i] = mix
            #print(pesudo_image.shape, pesudo_image)
            #print(fill_val) #(125.0, 125.0, 125.0)
            fill_val = fill_val[0]
            tmp[pesudo_image==1] = fill_val
            results[key] = tmp

    def _erase_mask(self, results, mix, patch, fill_val=0):
        for key in results.get("mask_fields", []):
            masks = results[key]
            if isinstance(masks, PolygonMasks):
                # convert mask to bitmask
            ...
            tmp = masks.masks.copy()
            c, h, w = tmp.shape
            pesudo_image = np.zeros((c, h, w))
            x1, y1, x2, y2 = patch
            for i in range(c):
                pesudo_image[i, y1:y2, x1:x2] = mix            
            #print("_erase_mask", fill_val) #0
            tmp[pesudo_image==1] = fill_val
            masks = BitmapMasks(tmp, masks.height, masks.width)
            results[key] = masks

    def _erase_seg(self, results, mix, patch, fill_val=0):
        for key in results.get("seg_fields", []):
            seg = results[key].copy()
...
@PIPELINES.register_module()
class MultiBranch(object):             #MultiBranch 输出是 list 而不是我们常用的 dict。
    def __init__(self, **transform_group):
        self.transform_group = {k: BaseCompose(v) for k, v in transform_group.items()}  # BaseCompose功能如下
