...
class FocalLossCost_1(nn.Module):
    """FocalLossCost.
    ...
    def __init__(self, weight=1., alpha=0.25, gamma=2, eps=1e-12):
        super(FocalLossCost_1, self).__init__()
        self.weight = weight
        self.alpha = alpha
        self.gamma = gamma
        self.eps = eps

    @force_fp32(apply_to=('cls_pred', 'gt_labels'))
...
@MATCH_COST.register_module(force=True)
class FocalLossCost_2(nn.Module):
    """FocalLossCost.
    ...
    def __init__(self, weight=1., alpha=0.25, gamma=2, eps=1e-12):
        super(FocalLossCost_2, self).__init__()
        self.weight = weight
        self.alpha = alpha
        self.gamma = gamma
        self.eps = eps
    ...
    @force_fp32(apply_to=('cls_pred', 'gt_labels'))
    def __call__(self, cls_pred, gt_labels, tea_scores):
        """
        Args:
            cls_pred (Tensor): Predicted classification logits, shape
                [num_query, num_class].
        ...
        tea_scores = tea_scores.expand(num_query, -1)
        cls_pred = cls_pred.sigmoid()[:, gt_labels]
        
        
        neg_cost = -(1 - cls_pred + self.eps).log() * (1-tea_scores) * (
            1 - self.alpha) * (tea_scores - cls_pred).pow(self.gamma)
        pos_cost = -(cls_pred + self.eps).log() * tea_scores * self.alpha * (
            tea_scores - cls_pred).pow(self.gamma)
        
        cls_cost = pos_cost - neg_cost
        return cls_cost * self.weight * 7
    ...
    def assign(self,
        ...
        if self.cls_cost.weight != 0 and cls_pred is not None:
            ...
            if gt_scores is not None:
                cls_cost_v1 = build_match_cost(dict(type='FocalLossCost_2', weight=2.0),)
                cls_cost = cls_cost_v1(cls_pred, gt_labels, gt_scores)
                
                #print("unlabel"+"-"*50)
                #print("cls_cost", cls_cost.cpu().detach().numpy())
            else:
                cls_cost = self.cls_cost(cls_pred, gt_labels)
                #print("label"+"|"*50)
                #print("cls_cost", cls_cost.cpu().detach().numpy())
        ...
        if self.mask_cost.weight != 0:
            ...
            if gt_ious is not None:
                #print("gt_ious", gt_ious)
                num_mask, h, w = bbox_pred.shape
                soft_weight = gt_ious.expand(num_mask, -1)
                reg_cost = reg_cost * soft_weight 
            #print("+"*100)
            '''
                print("unlabel"+"-"*50)
                print("reg_cost", reg_cost.cpu().detach().numpy())
            else:
                print("label"+"|"*50)
                print("reg_cost", reg_cost.cpu().detach().numpy())
            '''
        ...
        if self.dice_cost.weight != 0:
            ...
            if gt_ious is not None:
                num_mask, h, w = bbox_pred.shape
                soft_weight = gt_ious.expand(num_mask, -1)
                dice_cost = dice_cost * soft_weight
