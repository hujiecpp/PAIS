...
#将bg_mask剔除掉，不然会报错
                pos_inds = (labels_per_img >= 0) & (labels_per_img < 80)
                scores_per_img = scores_per_img[pos_inds]
                labels_per_img = labels_per_img[pos_inds]
                masks_per_img = masks_per_img[pos_inds]
                
#这边不进行二值化                
                """
                valid = scores_per_img > 0.35
...
#生成的mask的尺度-------------------------由于上一步的scale，是feature的两倍
                '''
                print("knet的预测"+"%"*100)
                print("masks_per_img.shape", masks_per_img.shape)
                print("feature.shape", x.shape)
                print("img_metas[img_id]['img_shape']", img_metas[img_id]['img_shape'])
                print("img_metas[img_id]['ori_shape']", img_metas[img_id]['ori_shape'])
                print("img_metas[img_id]['batch_input_shape']", img_metas[img_id]['batch_input_shape'])
                '''
                #print("tea在knet的head中的img"+"-"*50)
                #print("img_metas[img_id]['batch_input_shape']", img_metas[img_id]['batch_input_shape'])
                seg_masks = self.rescale_masks(masks_per_img, img_metas[img_id])
                
#先不二值化
                #seg_masks = seg_masks > self.test_cfg.mask_thr         # 0.5 将小于0.5的归为false, 其他为True, 即完成的mask的作用
                
#查看预测的mask的尺度跟img的尺度----------------------------------mask的大小为ori_shape的大小

                '''
                print("knet rescale之后的预测"+"%"*100)
